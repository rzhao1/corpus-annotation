...
import static edu.cmu.lti.oaqa.corpus_annotation.gerpphase.core.TokenizerHelper.findNextNonAlphaNum;
import static edu.cmu.lti.oaqa.corpus_annotation.gerpphase.core.TokenizerHelper.startsWithWithoutBeingFollowedByLetter;

import java.util.HashMap;

import edu.cmu.lti.oaqa.corpus_annotation.gerpphase.core.HyphenatedPTB;
...
public class ContractionsPTB {
  ...
  static String[] possibleContractionEndings = { "'s", "'ve", "'re", "'ll", "'d", "'n", "n't" }; 
  ...
  private static void test_getLengthIfNextApostIsMiddleOfContraction() {
    ...
    int expectedLenWordToken; // as opposed to expected length of the contraction token part // "ca"
                              // part of can't or "he" part of he'll
    ...
    nextNonCharDigitApostrophe = text.indexOf(APOSTROPHE, position); // function to use depends on
                                                                     // string
    ...
    System.out.println("INFO: for text = " + text + ", result = " + cr.getWordTokenLen() + " "
            + cr.getContractionTokenLen());
  ...
  /**
   * Determine if the text starting at 'position' within 'text' is the start of a contraction such
   * as "should've" or "hasn't" or "it's" by looking at whether there is a letter before the
   * apostrophe, and the appropriate letters after the apostrophe (or in the case of "n't", verify
   * the letter before is an 'n' Note that if the text starting at 'position' is something like
   * "n't" which isn't a complete word, returns null.
   * 
   * @param position
   *          first char of next token
   * @param lowerCasedText
   *          text into which parameter position is an index into
   * @return the length of the WordToken part of the contraction. Note this is not always the
   *         position of the apostrophe. For example, for can't, which is tokenized as ca n't the
   *         length is 2. For "it's", the length is also 2.
   * @see #lenOfFirstTokenInContraction for handling contractions like "cannot" that don't have an
   *      apostrophe
   */
  public static ContractionResult getLengthIfNextApostIsMiddleOfContraction(int position,
          int nextNonLetterDigit, String lowerCasedText) {

    if (position < 0)
      return null;
    if (lowerCasedText.length() < position + 3)
      return null; // need at least one letter after the apostrophe and one before ('tis and 'twas
                   // handled elsewhere)
    int apostrophePosition = lowerCasedText.indexOf(APOSTROPHE, position);

    // System.out.println("getLengthIfNextApostIsMiddleOfContraction: " + position + " " +
    // nextNonLetterDigit + " " + lowerCasedText);
    ...
    if (nextNonLetterDigit != apostrophePosition)
      return null;
    if (apostrophePosition < 1 || apostrophePosition >= lowerCasedText.length() - 1
            || lowerCasedText.startsWith("n't")) {
      return null;
    }

    // First just check the one character after the apostrophe before we start checking in more
    // detail
    // because we can rule out a lot of things this way
    String letterAfterApostrophe = lowerCasedText.substring(apostrophePosition + 1,
            apostrophePosition + 2);
    ...
    String restStartingWithApostrophe = lowerCasedText.substring(apostrophePosition,
            subseqentNonAlphaNum); // "'n he could do" or 'n or 've or 'll or 't
    ...
    loop: for (String s : possibleContractionEndings) {
      ...
      if (lowerCasedText.length() < apostrophePosition + lenAfterApostrophe)
        continue loop; // not enough text for this possibleContractionEndings to be a match

      // if (s.equals("'t")) { // different in that the contraction token includes character before
      // the apostrophe
      // if (rest.equals(s) && (prev=='n' || prev=='N')) {
      // throw new
      // UnsupportedOperationException("the n't case is supposed to be handled elsewhere");
      // //return text.length()-3; // TBD how to tell it to go back 1 for the n't case? -- don't --
      // the n't case is handled elsewhere
      // } else {
      // continue loop;
      // }
      // }
      ...
      // there's at least one character after, check that it isn't a letter or number, which would
      // be part of the same token
      // and would mean the apostrophe wasn't part of a contractiona after all. for example
      // "he'dr. smith" and "can'they" are
      // more likely the end of quoted sentences and teh start of a new sentence than a misspelled
      // contraction

      // we checked exact match above.
      // If same length as exact match but not an exact match, done with this one, go on
      if (lowerCasedText.length() == apostrophePosition + lenAfterApostrophe + 1)
        continue loop; // if not an exact match but has same length as exact match would, then not
                       // the right one
  ...
  //
  // static boolean isMultiWordToken(int currentPosition, String textSegment, int
  ...
  static boolean isContractionThatStartsWithApostrophe(int currentPosition, String lowerCasedText) {
    ...
    for (String s : contractionsStartingWithApostrophe) {
      if (startsWithWithoutBeingFollowedByLetter(lowerCasedSubstring, s))
        return true;
    }
    return false;
  }
  ...
  // Find the 3 characters that are the next possible token breaks (look for next 3 whitespace,
  // punctuation, but (*) count contiguous whitespace as one)
  // We are most interested in those that sometimes cause a split and sometimes don't -- apostrophes
  // and hyphens.
  // 80's-esque should be one token according to the 2 rules. (potential break characters for that
  // example
  // are the apostrophe, the hyphen, and the whitespace)
  // salon-o-torium should be one token, with a single contraction token and a single word token
  // (potential
  ...
  // apostr hyphen hyphen test for -o-torium, otherwise take at most up to 3rd break (ignore case of
  // o-torium followed by something more meaningful)
  ...
  // Note that an exception prefix between apostrophe and 1st hyphen does not avoid the break at the
  // hyphen
  // so the logic is not something that can be broken down into just looking at the not splitting at
  // the
  // apostrophe followed by normal hyphen processing.

  private static String[] fullWordsNotToBreakAtApostrophe = { "p'yongyang", };

  /**
   * Assumes apostrophe is not first character.... that case is handled elsewhere Assumes
   * <code>s</code> is lower case.
   */
  static boolean breakAtApostrophe(String s, int positionOfApostropheToTest) {
    ...
    if (allDigits(s.substring(0, positionOfApostropheToTest))
            && s.charAt(positionOfApostropheToTest + 1) == 's') {
      if (s.length() < positionOfApostropheToTest + 3) {
        return false; // 80's<end_of_input>
      }
      // Check that after the 's there aren't more letters or digits which would be unknown like 'st
      // or 's2
  ...
  /**
   * for a word like 80's or P'yongyang or James' or Sean's or 80's-like or 80's-esque (or can't or
   * haven't, which are to be split) determine whether the singlequote(apostrophe) needs to be kept
   * with the surrounding letters/numbers and what to do about hyphenated afterwards if there is a
   * hyphen after.... For possessives, do split. Note that things that start with an apostrophe like
   * 'Assad were handled elsewhere
   * 
   * @return len of how much to keep: len to apostrophe, or to next breaking char (the space after s
   *         for "80's ") or end of hyphenated suffix that should also remain attached, or -1
   */
  static int tokenLengthCheckingForSingleQuoteWordsToKeepTogether(String lowerCasedText) {
    ...
    if (firstBreak < 0)
      throw new UnsupportedOperationException("no quote/apostrophe char found in '"
              + lowerCasedText + "'");
    if (firstBreak == 0)
      return -1;
    ...
    // else going to keep at least past the apostrophe, but if there's a hyphenated word or a
    // hyphenated suffix that should not be split,
    // keep that much too
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < MultiTokenWords.length; i++) {
      ...
      String SPACE = " ";
      System.out.println(s.substring(0, MultiTokenWordLenToken1[i])
              + SPACE
              + s.substring(MultiTokenWordLenToken1[i], MultiTokenWordLenToken1[i]
                      + MultiTokenWordLenToken2[i]) + SPACE
