...
public class DependencyPath extends LinkedList<ConllDependencyNode> {
  ...
  public String toString() {
    ...
    for (ConllDependencyNode node : this) {
      ...
      str.append(foundCommon ? DN_ARC_A + L_REL_DELIM + node.getDeprel() + R_REL_DELIM + DN_ARC_B
              + L_TOK_DELIM + node.getForm() + R_TOK_DELIM + L_POS_DELIM + node.getPostag()
              + R_POS_DELIM : L_TOK_DELIM + node.getForm() + R_TOK_DELIM + L_POS_DELIM
              + node.getPostag() + R_POS_DELIM + UP_ARC_B + L_REL_DELIM + node.getDeprel()
              + R_REL_DELIM + UP_ARC_A);
    }
    return str.toString();
  }
  ...
  public String toTripleString() {
    ...
    for (ConllDependencyNode node : this) {
      ...
      str.append(foundCommon ? L_TOK_DELIM + node.getHead().getForm() + R_TOK_DELIM + L_POS_DELIM
              + node.getHead().getPostag() + R_POS_DELIM + DN_ARC_A + L_REL_DELIM
              + node.getDeprel() + R_REL_DELIM + DN_ARC_B + L_TOK_DELIM + node.getForm()
              + R_TOK_DELIM + L_POS_DELIM + node.getPostag() + R_POS_DELIM : L_TOK_DELIM
              + node.getForm() + R_TOK_DELIM + L_POS_DELIM + node.getPostag() + R_POS_DELIM
              + UP_ARC_B + L_REL_DELIM + node.getDeprel() + R_REL_DELIM + UP_ARC_A + L_TOK_DELIM
              + node.getHead().getForm() + R_TOK_DELIM + L_POS_DELIM + node.getHead().getPostag()
              + R_POS_DELIM
  ...
  public String toSyntaxString() {
    ...
    for (ConllDependencyNode node : this) {
      ...
      str.append(foundCommon ? DN_ARC_A + L_REL_DELIM + node.getDeprel() + R_REL_DELIM + DN_ARC_B
              + L_POS_DELIM + node.getPostag() + R_POS_DELIM : L_POS_DELIM + node.getPostag()
              + R_POS_DELIM + UP_ARC_B + L_REL_DELIM + node.getDeprel() + R_REL_DELIM + UP_ARC_A);
    }
    return str.toString();
  }
