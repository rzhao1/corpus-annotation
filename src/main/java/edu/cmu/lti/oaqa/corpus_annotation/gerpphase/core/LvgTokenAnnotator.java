...
import org.oaqa.model.kb.Lemma;
import org.oaqa.model.nlp.Segment;
import org.oaqa.model.nlp.Token;

/**
 * UIMA annotator that uses the UMLS LVG package to find the canonical form of BaseTokens. The
 * package is also used to find one or more lemmas for a given BaseToken along with its associated
 * part of speech.
 * 
 * @author Ran Zhao
 * 
 *         to do: what effect does using the cache have on words that may be misspelled. It seems
 *         that if you automatically normalize a word from the cache, this may be bad if it is
 *         misspelled in the case where the misspelling is a word in the lexicon. 
 * Phase: LvgToken Annotator
 */
public class LvgTokenAnnotator extends JCasAnnotator_ImplBase {
  /**
   * Value is "PostLemmas". This parameter determines whether the feature lemmaEntries will be
   * populated for word annotations.
   */
  public static final String PARAM_POST_LEMMAS = "PostLemmas";

  /**
   * Value is "UseLemmaCache". This parameter determines whether a cache will be used to improve
   * performance of setting lemma entries.
   */
  ...
  /**
   * Value is "LemmaCacheFrequencyCutoff". This parameter sets a threshold for the frequency of a
   * lemma to be loaded into the cache.
   */
  ...
  public void initialize(UimaContext aContext) throws ResourceInitializationException {
    ...
    try {
      ...
      String Uri = getClass().getClassLoader()
              .getResource((String) context.getConfigParameterValue("ModelFilePath")).getPath();
      LvgCmdApiResourceImpl lvgResource = new LvgCmdApiResourceImpl();
      lvgResource.load(Uri);
      if (lvgResource == null)
        throw new AnnotatorInitializationException(new Exception(
                "Unable to locate resource with key=" + LVGCMDAPI_RESRC_KEY + "."));
  ...
  private void configInit() throws ResourceInitializationException {
    ...
    // System.out.println("######"+skipSegmentIDs[i]);
    ...
    if (postLemmas) {
      ...
      if (useLemmaCache) {
        ...
        if (lemmaCacheFileLocation == null)
          throw new ResourceInitializationException(new Exception("Parameter for "
                  + PARAM_LEMMA_CACHE_FILE_LOCATION + " was not set."));
   ...
   */
  protected void annotateRange(JCas jcas, String text, int rangeBegin, int rangeEnd)
          throws AnalysisEngineProcessException {
    JFSIndexRepository indexes = jcas.getJFSIndexRepository();
    Iterator tokenItr = indexes.getAnnotationIndex(Token.type).iterator();
    while (tokenItr.hasNext()) {
      Token tokenAnnotation = (Token) tokenItr.next();
      if (tokenAnnotation.getBegin() >= rangeBegin && tokenAnnotation.getEnd() <= rangeEnd) {
        ...
        setNormalizedForm(tokenAnnotation, token, jcas);
        if (postLemmas)
  ...
  private void setNormalizedForm(Token tokenAnnotation, String token, JCas jcas)
          throws AnalysisEngineProcessException {
    ...
    if (normalizedForm != null) {
      tokenAnnotation.setLemmaForm(normalizedForm);
      if (tokenAnnotation.getGenerators() == null) {
        NonEmptyStringList generator = new NonEmptyStringList(jcas);
        tokenAnnotation.setGenerators(generator);
        generator.setHead("Lemma: LVG");
      } else {

        NonEmptyStringList generator = (NonEmptyStringList) tokenAnnotation.getGenerators();
        while (generator.getTail() != null) {
  ...
  private void setLemma(Token wordAnnotation, String word, JCas jcas)
          throws AnalysisEngineProcessException {
