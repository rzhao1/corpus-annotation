 ...
 * 
 * The original reason for this annotator is to extend NP annotations to include prepositional
 * phrases so that for the pattern NP PP NP, named entities that includes a word(s) from each of
 * those NPs is found.
 * 
 * Searches for the pattern within each Sentence. The end offset of the first chunk in the pattern
 * is extended to match the end offset of the last chunk in the pattern.
 * 
 * Note the pattern is applied repeatedly so that a sentence of NP PP NP PP NP results in only the
 * first NP being extended all the way to the last NP in that sentence. This prevents NP annotations
 * from only partially overlapping other NP annotations.
 * 
 ...
 * @author Ran Zhao
 * 
 *         Phase: AdjustNounPhraseToIncludeFollowingNP/AdjustNounPhraseToIncludeFollowingPPNP
 *         Annotator
 * 
 */
public class ChunkAdjuster extends JCasAnnotator_ImplBase {
  ...
  /**
   * The index of the token (within the pattern) to extend the end offset to include. E.g. is 2 to
   * extend the first NP to include the last NP in NP PP NP.
   */
   ...
   * parameters. This method is not invoked for every document processed.
   * 
  ...
  private void configInit() throws ResourceInitializationException {
    ...
    indexOfTokenToInclude = ((Integer) context
            .getConfigParameterValue(PARAM_EXTEND_TO_INCLUDE_TOKEN)).intValue();
    ...
    if (indexOfTokenToInclude < 0 || indexOfTokenToInclude >= chunksTypesInPattern.length) {
      ...
      throw new ResourceInitializationException(
              AnnotatorConfigurationException.PARAMETER_NOT_VALID, msgArgs);
    }

  }

  /**
   * Invokes this annotator's analysis logic. Invoked for each document processed. For each
   * Sentence, look for the pattern, and adjust a chunk if the pattern is found.
   */
  ...
  /**
   * Remove from our local list of chunks the chunks that have been enveloped. This allows the rule
   * to be applied again.
   * 
   */
   ...
   * @return true if the pattern is matched by the chunks starting with element <code>i</code> in
   *         the list. Note if there aren't enough chunks in the list starting at i to match the
   ...
   * @throws AnnotatorProcessException
   */
  private Chunk extendChunk(Chunk chunk, int newEnd) throws AnalysisEngineProcessException {
