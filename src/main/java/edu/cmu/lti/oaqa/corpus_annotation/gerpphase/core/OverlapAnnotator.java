...
public class OverlapAnnotator extends JCasAnnotator_ImplBase {
  ...
  private Action buildModifyAction(String[] argArr) throws Exception {
    ...
    if (selectorStr != null) {
      if (selectorStr.equalsIgnoreCase("A")) {
        ...
        throw new Exception("ModifyAction selector invalid: " + selectorStr);
      }
    } else {
  ...
  public void process(JCas jcas) throws AnalysisEngineProcessException {
    ...
    List<GerpAnnotation> aAnnotList = storeAnnotationsToList(indexes, iv_aAnnotType, jcas);
    List<GerpAnnotation> bAnnotList = storeAnnotationsToList(indexes, iv_bAnnotType, jcas);

    Iterator<GerpAnnotation> aAnnotItr = aAnnotList.iterator();
    while (aAnnotItr.hasNext()) {
      GerpAnnotation aAnnot = aAnnotItr.next();

      // for each A, iterate over B annotations
      Iterator<GerpAnnotation> bAnnotItr = bAnnotList.iterator();
      while (bAnnotItr.hasNext()) {
        GerpAnnotation bAnnot = bAnnotItr.next();
   ...
   * @return
   */
  private List<GerpAnnotation> storeAnnotationsToList(JFSIndexRepository indexes, int annotType,
          JCas jcas) {
    List<GerpAnnotation> l = new ArrayList<GerpAnnotation>();
    FSIterator<Annotation> annotItr = indexes.getAnnotationIndex(annotType).iterator();
    while (annotItr.hasNext()) {
      GerpAnnotation annot = (GerpAnnotation) annotItr.next();
      if (annot.getGenerators() == null) {
        NonEmptyStringList generator = new NonEmptyStringList(jcas);
        annot.setGenerators(generator);
        generator.setHead("cTakes lookup");
      } else {
        NonEmptyStringList generator = (NonEmptyStringList) annot.getGenerators();
   ...
   */
  private BitSet getOverlapType(GerpAnnotation a, GerpAnnotation b) {
  ...
  /**
   * Generic interface for performing an action if the overlap constraint is met between annotations
   * A and B.
   * 
   */
  interface Action {
    public void performAction(GerpAnnotation aAnnot, GerpAnnotation bAnnot);
  }

  /**
   * Implementation that modifies feature values of either annotation A or B. Currently supports
   * modifying begin offset and end offset.
   */
  class ModifyActionImpl implements Action {
    ...
    private void modifyBegin(GerpAnnotation ann, GerpAnnotation aAnnot, GerpAnnotation bAnnot) {
      switch (iv_beginSrc) {
        case BEGIN_A:
    ...
    private void modifyEnd(GerpAnnotation ann, GerpAnnotation aAnnot, GerpAnnotation bAnnot) {
      switch (iv_endSrc) {
        case END_A:
