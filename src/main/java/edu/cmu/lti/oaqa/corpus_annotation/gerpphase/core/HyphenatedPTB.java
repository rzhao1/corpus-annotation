...
public class HyphenatedPTB {
  ...
  static String[] possibleContractionEndings = { "'s", "'ve", "'re", "'ll", "'d", "'n", "n't" }; // note
                                                                                                 // 't
                                                                                                 // is
                                                                                                 // different
                                                                                                 // in
  ...
  // uni-
  // vice-
  static String[] hyphenatedPrefixes = { "e-", "a-", "u-", "x-", "agro-", "ante-", "anti-",
      "arch-", "be-", "bi-", "bio-", "co-", "counter-", "cross-", "cyber-", "de-", "eco-", "ex-",
      "extra-", "inter-", "intra-", "macro-", "mega-", "micro-", "mid-", "mini-", "multi-", "neo-",
      "non-", "over-", "pan-", "para-", "peri-", "post-", "pre-", "pro-", "pseudo-", "quasi-",
      "re-", "semi-", "sub-", "super-", "tri-", "ultra-", "un-", "uni-", "vice-",
  ...
  static String[] hyphenatedSuffixes = { "-esque", "-ette", "-fest", "-fold", "-gate", "-itis",
      "-less", "-most", "-o-torium", "-rama", "-wise" };
   ...
   * 
   * And here are some made-up examples of words using affixes to keep together chronic-itis 1
   * suffix mega-huge 1 prefix e-game-fest 1 prefix and 1 suffix salon-o-torium 1 suffix that
   * contains 2 hyphens urban-esque-wise 2 suffixes
   * 
   * @param lowerCasedString
   *          because of "-o-torium", input might contain more than 1 hyphen....
   * @return len to keep together, as far as we know. see hyphen hyphen hyphen case below. throws
   *         exception if there's no hyphen; number of characters to keep. Does not mean to split at
   *         n+1 hyphen... need to recheck that one
   * 
   */
  public static int tokenLengthCheckingForHyphenatedTerms(String lowerCasedString) {
    ...
    if (firstBreak + 1 == lowerCasedString.length())
      return firstBreak; // if ends with hyphen, don't include the hyphen in the token. mega- by
                         // itself should be mega and -
    ...
    // We are most interested in those that sometimes cause a split and sometimes don't --
    ...
    // hyphen hyphen hyphen test for o-torium, otherwise take at most up to 2nd break, but check
    // rest next time (TBD)
    ...
    if (secondBreak == lowerCasedString.length()) {
      return lenIncludingHyphensToKeep(lowerCasedString, firstBreak, 1, secondBreak, thirdBreak); // determines
                                                                                                  // if
                                                                                                  // we
                                                                                                  // should
                                                                                                  // we
      ...
      return lenIncludingHyphensToKeep(lowerCasedString, firstBreak, 2, secondBreak, thirdBreak); // take
                                                                                                  // up
                                                                                                  // to
                                                                                                  // 2nd
                                                                                                  // or
      ...
      return lenIncludingHyphensToKeep(lowerCasedString, firstBreak, 1, secondBreak, thirdBreak); // take
                                                                                                  // up
                                                                                                  // to
                                                                                                  // 2nd
                                                                                                  // break
  ...
  private static int lenIncludingHyphensToKeep(String s, int indexOfFirstHyphen,
          int numberOfHyphensToConsiderKeeping, int secondBreak, int thirdBreak) {
    ...
    if (numberOfHyphensToConsiderKeeping > 2 || numberOfHyphensToConsiderKeeping < 1) {
      throw new UnsupportedOperationException(
              "Not ready to handle numberOfHyphensToConsiderKeeping = "
                      + numberOfHyphensToConsiderKeeping);
    }
    ...
    // Now either numberOfHyphensToConsiderKeeping==1 or was ==2 but no 2-hyphen suffix was found to
    // match
    // Try one-hyphen suffixes, either just 1 of them or 2 of them (but not yet checking for 1 with
    // a prefix too, see prefixes section below for that...
    ...
    if (lookup) { // First hyphen is start of a suffix that should not be split off
      // could be numberOfHyphensToConsiderKeeping==1 here, or could be 2 separate 1-hyphen suffixes
      // are both
    ...
    if (numberOfHyphensToConsiderKeeping > 1) {
      ...
      lookup = hyphenatedWordsLookup.contains(possibleHyphenatedWordsLookupMatch)
              && hyphenatedSuffixesLookup.contains(possibleSuffix);
      if (lookup)
        return thirdBreak;
    }
    ...
    if (lookup && numberOfHyphensToConsiderKeeping > 1) {
      String possibleHyphenatedWordsLookupMatch = s.substring(indexOfFirstHyphen + 1, thirdBreak); // e.g.
                                                                                                   // uh-oh
    ...
    if (numberOfHyphensToConsiderKeeping == 2) {
      if (lookup) { // a prefix was found that should not be split
        ...
        if (lookup2)
          return thirdBreak; // both a prefix and a suffix that are not to be split, keep all
                             // together
        ...
        // String m = "This condition checked already in other if-else " + indexOfFirstHyphen +
        // COMMA + secondBreak + COMMA + thirdBreak + COMMA + s;
