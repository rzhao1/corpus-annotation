...
import static edu.cmu.lti.oaqa.corpus_annotation.gerpphase.core.TokenizerHelper.APOSTROPHE;
import static edu.cmu.lti.oaqa.corpus_annotation.gerpphase.core.TokenizerHelper.COMMA;
import static edu.cmu.lti.oaqa.corpus_annotation.gerpphase.core.TokenizerHelper.CR;
import static edu.cmu.lti.oaqa.corpus_annotation.gerpphase.core.TokenizerHelper.HYPHEN_OR_MINUS_SIGN;
import static edu.cmu.lti.oaqa.corpus_annotation.gerpphase.core.TokenizerHelper.NEWLINE;
import static edu.cmu.lti.oaqa.corpus_annotation.gerpphase.core.TokenizerHelper.PERIOD;
import static edu.cmu.lti.oaqa.corpus_annotation.gerpphase.core.TokenizerHelper.isPunctuation;
...
public class TokenizerPTB {
   ...
   *          what to add to all offsets within textSegment to make them be offsets from the start
   *          of the text for the jcas
   * @param includeTextNotJustOffsets
   *          whether to copy the text covered by this token into the token object itself
   * @return the list of new tokens
   */
  public List<?> tokenizeTextSegment(JCas jcas, String textSegment, int offsetAdjustment,
    ...
    while ((currentPosition = findFirstCharOfNextToken(textSegment, currentPosition)) >= 0) {
      ...
      // else we have at least 2 characters to consider
        ...
        if (len > 0) {
          ...
          // Abbreviation does not start with period, and not part of some other token, so it is
          // punctuation
          tokenLen = 1;
          tokenClass = PunctuationToken.class;
        }
        ...
        } else if (ContractionsPTB.isContractionThatStartsWithApostrophe(currentPosition,
                lowerCasedText)) {
        ...
        // First check the easy case - if just letters and digits until next whitespace (or until
        // end of segment)
        ...
        do {
          ...
          if (isWhitespace(ch)) {
            ...
            obviouslyIsNumber = false; // not sure if it will be number all the way to whitespace
            // since passed nextNonLetterOrNonDigit test above, must be letter, so
            // nextNonLetterOrNonDigit is not changed here
            // since passed !isLetterOrDigit test above, must be letter, so
        ...
        if (obviouslyIsNumber) {
          ...
          String lowerCasedSubstring = textSegment.substring(currentPosition,
                  nextWhitespaceOrEndOfSegment).toLowerCase();
          int len = ContractionsPTB.lenOfFirstTokenInContraction(lowerCasedSubstring);
          if (len > 0) { // is a contraction that doesn't contain an apostrophe, like "gonna",
                         // create WordToken for first part,
            // and create ContractionToken for other token(s)
            tokenLen = len;
            tokenClass = WordToken.class;
            bta = createToken(tokenClass, textSegment, jcas, currentPosition, currentPosition
                    + tokenLen, offsetAdjustment);
            tokens.add(bta);
            ...
            if (len > 0) { // if there is a 3rd, create the 2nd and set up for the 3rd to be created
                           // later
              bta = createToken(tokenClass, textSegment, jcas, currentPosition, currentPosition
                      + tokenLen, offsetAdjustment);
              tokens.add(bta);
        ...
        } else { // Still within the "isLetterOrDigit(firstCharOfToken)" but not obviously number or
                 // word
          ...
          // Need to check for things like 80's before checking for contractions or else 80's looks
          // like a contraction
          if (nextNonLetterOrNonDigit < lowerCasedText.length()
                  && lowerCasedText.charAt(nextNonLetterOrNonDigit) == APOSTROPHE) {
            String lowerCasedSubstring = lowerCasedText.substring(currentPosition,
                    nextWhitespaceOrEndOfSegment);
            len = ContractionsPTB
                    .tokenLengthCheckingForSingleQuoteWordsToKeepTogether(lowerCasedSubstring);
            if (len > nextNonLetterOrNonDigit - currentPosition) { // if keeping the apostrophe
                                                                   // attached
              tokenLen = len;
          ...
          if (tokenLen == NOT_SET_INDICATOR) { // not found yet
            if ((cr = ContractionsPTB.getLengthIfNextApostIsMiddleOfContraction(currentPosition,
                    nextNonLetterOrNonDigit, lowerCasedText)) != null) {
              ...
              if (c == 'n' || c == APOSTROPHE) { // if a "n't" contraction or a contraction where
                ...
                if (tokenLen < 0)
                  throw new RuntimeException("c = " + c + "tokenLen = " + tokenLen
                ...
                // Set up to create the second token, for other contractions, the next token will
                // start with an
                // apostrophe and be handled above... but for "n't" contractions, next token won't
                // start with apostrophe
                ...
                // if (tokenLen!=3) throw new
                // RuntimeException("getContractionTokenLen != 3 for n't");
                tokenClass = ContractionToken.class;
              } else {
                throw new RuntimeException(
                        "ERROR: getLengthIfNextApostIsMiddleOfContraction returned " + len
                                + " but the character (" + c
                                + ") after that is not 'n' or apostrophe ");
              }

            } else if ((len = lenIfIsTelephoneNumber(currentPosition, lowerCasedText,
                    nextNonTelephoneOrPostalChar)) > 0) {
              tokenLen = len;
              tokenClass = WordToken.class;
            } else if ((len = lenIfIsPostalCode(currentPosition, lowerCasedText,
                    nextNonTelephoneOrPostalChar)) > 0) {
              tokenLen = len;
              tokenClass = WordToken.class;
            } else if ((len = lenIfIsUrl(currentPosition, lowerCasedText,
                    nextWhitespaceOrEndOfSegment)) > 0) {
              tokenLen = len;
              tokenClass = WordToken.class;
            } else if ((len = lenIfIsEmailAddress(currentPosition, lowerCasedText,
                    nextWhitespaceOrEndOfSegment)) > 0) {
              tokenLen = len;
              tokenClass = WordToken.class;
            } else if ((len = lenIfIsAbbreviation(currentPosition, textSegment,
                    nextWhitespaceOrEndOfSegment)) > 0) {
              tokenLen = len;
              tokenClass = WordToken.class;
              ...
              // or apostrophe like in 80's or P'yongyang (one token each) or James' or Ted's (2
              // tokens each)
              ...
              // System.out.println("nextWhitespaceOrEndOfSegment = " +
              // nextWhitespaceOrEndOfSegment);
              // System.out.println("nextNonLetterOrNonDigit = " + nextNonLetterOrNonDigit);
              // System.out.println("nextNonLetterDigitApostrophe = " +
              // nextNonLetterDigitApostrophe);
              // }

              if (nextNonLetterOrNonDigit < lowerCasedText.length()
                      && lowerCasedText.charAt(nextNonLetterOrNonDigit) == HYPHEN_OR_MINUS_SIGN) {
                ...
                tokenLen = len;
                if (tokenLen < 0)
                  throw new RuntimeException("tokenLen = " + tokenLen + " currentPosition = "
                          + currentPosition + " nextNonLetterOrNonDigit = "
                          + nextNonLetterOrNonDigit);
                tokenClass = wordTokenOrNumToken(lowerCasedText, currentPosition, tokenLen);
              } else if (nextNonNumericChar > 0
                      && (len = lenIfIsNumberContainingComma(currentPosition, lowerCasedText,
                              nextNonNumericChar)) > 0) {
                tokenLen = len;
                tokenClass = NumToken.class;
              } else if (nextNonLetterDigitApostrophe < lowerCasedText.length()
                      && lowerCasedText.charAt(nextNonLetterDigitApostrophe) == PERIOD) {
                // see if is a number with a decimal place (without commas, comma-containing numbers
                // are handled above)
                if (nextNonDigit == lowerCasedText.length() - 1) {
                  // end of sentence, don't include the period as part of the number, count it as
                  // end of sentence marker (punctuation)
                  tokenLen = nextNonDigit - currentPosition;
                  // if (tokenLen<1) throw new RuntimeException("Period at end of sentence " +
                  // nextNonDigit + " " + nextNonLetterDigitApostrophe+" "+tokenLen+ " " +
                  // lowerCasedText);
                  tokenClass = NumToken.class;
                } else if (nextNonLetterDigitApostrophe == nextNonDigit) {
                  // if not end of sentence, do include period (decimal point) in the NumToken
                  tokenLen = nextNonDigit + 1
                          + getLenToNextNonDigit(lowerCasedText, nextNonDigit + 1)
                          - currentPosition;
                  tokenClass = NumToken.class;
                } else {
              ...
              // } else {
              // throw new UnsupportedOperationException("nextNonLetterDigitApostrophe = " +
              // nextNonLetterDigitApostrophe);
              // }
  ...
  private int lenIfIsNumberContainingComma(int currentPosition, String text, int nextNonNumericChar) {
    String s = text.substring(0, nextNonNumericChar); // use substring so don't search until end of
                                                      // entire document
    ...
    // the whole number part can contain commas as long as there are exactly 3 digits after each
    // comma
    if (commaPosition > endOfWholeNumberPart)
      return -1; // if comma appears after the decimal point, then no commas in the
                 // whole-number-part
    if (commaPosition == 0)
      return -1; // can't start with comma
    ...
    while (!didNotFindExactlyThreeDigitsAfterComma) {
      len = position - currentPosition; // don't include the comma unless also can include next 3
                                        // digits
      ...
      if (position < endOfWholeNumberPart && isDigit(s.charAt(position))) { // can't have 4 digits
                                                                            // after comma like
                                                                            // 3,4567
        didNotFindExactlyThreeDigitsAfterComma = true;
      }
    ...
    // But if the sentences ends with the period that follows the whole_number_part, count it as the
    // sentence marker
    // not as part of the number
    if (periodPosition != text.length() - 1 && // not the final period of a sentence
            periodPosition == currentPosition + len) { // but the period does appear right after the
                                                       // whole_number_part
      len++;
  ...
  private int lenIfIsPostalCode(int currentPosition, String text, int nextNonPostalCodeChar) {
    ...
    if (len == 10) { // 55901-0001
      ...
      if (!isDigit(s.charAt(1)))
        return -1;
      if (!isDigit(s.charAt(2)))
        return -1;
      if (!isDigit(s.charAt(3)))
        return -1;
      if (!isDigit(s.charAt(4)))
        return -1;
      if (s.charAt(5) != DASH)
        return -1;
      if (!isDigit(s.charAt(6)))
        return -1;
      if (!isDigit(s.charAt(7)))
        return -1;
      if (!isDigit(s.charAt(8)))
        return -1;
      if (!isDigit(s.charAt(9)))
        return -1;
      return len;
    } else {
      return -1;
  ...
  private int lenIfIsTelephoneNumber(int currentPosition, String text,
    ...
    if (len == 6) {
      ...
      if (!isDigit(s.charAt(11)))
        return -1;
      if (!isDigit(s.charAt(12)))
        return -1;
      if (!isDigit(s.charAt(13)))
        return -1;
      return len;
    } else {
      return -1;
  ...
  private Class<? extends BaseToken> wordTokenOrNumToken(String lowerCasedText,
          int currentPosition, int tokenLen) {
   ...
   * @param currentPosition
   * @param tokenLen
   * @return true if at least one of the characters between currentPosition and
   *         currentPosition+tokenLen is a letter
   */
  ...
  private boolean isEllipsis(int currentPosition, String textSegment) {
    if (textSegment.substring(currentPosition).startsWith(ellipsis))
      return true;
    return false;
  }
  ...
  private int getLengthIfNameStartingWithApostrophe(int currentPosition, String textSegment) {
    ...
    // Could be the start of a quoted string like "'The boy ran', she said" or could be the start of
    // a name like 'Assad
  ...
  /**
   * Assumes no white space between currentPosition and endOfInputToConsider If last of a sentence
   * is a period, then don't include the period with the abbreviation, count it as punctuation. That
   * way we don't have to differentiate between "mg." being an abbreviation and "me." being simply
   ...
   * @return
   */
  private int lenIfIsAbbreviation(int currentPosition, String mixedCaseText,
          int afterEndOfInputToConsider) {
    // Determine if all up to endOfInputToConsider contains at least 1 letter and ends with period
    // Note input is known to contain at least 1 letter or otherwise would have already been
    // determined to be a number
    ...
    if (afterEndOfInputToConsider - currentPosition >= 4
            && mixedCaseText.substring(currentPosition, currentPosition + 4).toLowerCase()
                    .equals("www.")) {
      return -1;
    }
    for (int i = currentPosition; i < afterEndOfInputToConsider; i++) {
      ...
      if (isLetter(ch)) {
        containsLetter = true;
      } else if (ch != PERIOD) { // if any symbol is found before the period, not considering it an
                                 // abbreviation
        return -1;
      } else if (!containsLetter || (i + 1 == mixedCaseText.length())) {
        return -1; // no letter, or last character of sentence is this period, in which case period
                   // is end of sentence marker, not part of abbreviation
      } else { // is a period and there was a letter before it and this period is not last char in
               // sentence
        ...
        if (Character.isWhitespace(peekAhead) || isPossibleFinalPunctuation(peekAhead)) {
          ...
          return soFar;
        } else if (!isLetterOrDigit(peekAhead)) { // "e.g.[1]" does have the abbreviation e.g.
                                                  // within it
          return soFar - 1;
        }
  ...
  private boolean isPossibleFinalPunctuation(char c) {
    if (possibleFinalPunctuation.indexOf(c) > -1)
      return true;
    return false;
  }

  private String validOtherEmailAddressCharacters = "!#$%&'*+/=?^_`{|}~-"; // those that can be used
                                                                           // without quoting or
                                                                           // escaping them
   ...
   * @return
   */
  private int lenIfIsEmailAddress(int currentPosition, String lowerCasedText,
    ...
    if (indexOfAt < 1 || currentPosition + indexOfAt + 1 == endOfInputToConsider
            || indexOfAt > maxLenLocalPart) { // '@' can't be the first character, but must be
                                              // present, and can't be last char
      // if no @ sign, or not in a valid position, don't bother doing anything more complicated,
      // can't
      // be an email address
      return -1;
    }
    ...
    for (int i = currentPosition; i < currentPosition + indexOfAt; i++) {
      ...
      if (ch == PERIOD && (i == currentPosition || i == currentPosition + indexOfAt - 1)) { // first
                                                                                            // and
                                                                                            // last
                                                                                            // of
                                                                                            // local
    ...
    char prev = '@';
    // The local part appears to be the right format for a valid email address, validate the domain
    // part
  ...
  private Class<? extends BaseToken> determineTokenType(String s, int begin, int end) {
    ...
    if (s == null || s.length() < end || begin + 1 != end) {
      Exception e = new Exception("ERROR: s not at least one char:  s= " + s + " begin, end = "
              + begin + "," + end);
   ...
   * 
   * @see org.apache.ctakes.core.ae.TokenConverter#convert(org.apache.ctakes.core.nlp.tokenizer.Token,
   *      org.apache.uima.jcas.JCas, int)
   */
  private Object createToken(Class<? extends BaseToken> clas, String s, JCas jcas, int begin,
          int end, int offsetAdjustment) {
    ...
    // System.out.println("The current token class is "+clas.getName());
    ...
    if (clas == null) { // determine the type for the caller
      ...
      //
      if (jcas != null)
        // token = new Token(jcas, beginFromStartOfDocument, endFromStartOfDocument);
        // ((Token)token).setTokenType(clss.getName());
      ...
      token = new Token(jcas, beginFromStartOfDocument, endFromStartOfDocument);
      if (clas.equals(NewlineToken.class)) {
        ((Token) token).setTokenType("NewlineToken");
      } else if (clas.equals(NumToken.class)) {
        ((Token) token).setTokenType("NumToken");
      } else if (clas.equals(WordToken.class)) {
        ((Token) token).setTokenType("WordToken");
      } else if (clas.equals(SymbolToken.class)) {
        ((Token) token).setTokenType("SymbolToken");
      } else if (clas.equals(PunctuationToken.class)) {
        ((Token) token).setTokenType("PunctuationToken");
      } else if (clas.equals(ContractionToken.class)) {
        ((Token) token).setTokenType("ContractionToken");
      } else if (clas.equals(BaseToken.class)) {
        ((Token) token).setTokenType("BaseToken");
      }

    }
    // else {
    // token = new Token(jcas, begin, end);
    //
    // }
    //
    // } else
  ...
  private void setNumType(NumToken nta, String tokenText) {
    if (org.apache.ctakes.core.nlp.tokenizer.Tokenizer.isNumber(tokenText)
            && !tokenText.contains(".")) {
  ...
  /*
   * Find the index of the first character of the next token, where the index is >= startPosition,
   * and the previous token ended at startPosition-1 (or there was no previous token for the 1st
   * time) Returns -1 if there are no more tokens (eof or all white space but no newlines)
   */
  ...
  static String[] testsForNumbers = { "2,000,123.For", "92,000,123.", "2,000,123.", "2,000,123.0",
      "2,000,13", "2", "2.", "2,", "22", "12345678901@4", "2.2.2." };

  static String[] testsForEmailAddress = { "masanz@mayo.edu", "masanz@mayo", "m@l", "m.@p",
      "m.n.@p", "3@4", "%@f", "R@@", "MASANZ@MAYO", "jk$jk@.m", "asdf@.m$",
      "masanz.james-mi@ibm.com.us", ".mn@p", ".@p", "@t", };
  ...
  static void runEmailTests() {
    ...
    for (String s : testsForEmailAddress) {
      ...
      String prepend = "XYZ";
      int j = tester.lenIfIsEmailAddress(prepend.length(), prepend + s,
