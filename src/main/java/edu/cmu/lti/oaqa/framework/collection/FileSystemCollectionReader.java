...
public class FileSystemCollectionReader extends AbstractCollectionReader {
  ...
  /*
   * 
   * @Override public void getNext(CAS aCAS) throws IOException,
   * CollectionException { JCas jcas; super.getNext(aCAS); try { jcas =
   * aCAS.getJCas(); } catch (CASException e) { throw new
  ...
  protected DataElement getNextElement() throws Exception {
    ...
    // if (mLanguage != null) {
    // ((DocumentAnnotation)
    // jcas.getDocumentAnnotationFs()).setLanguage(mLanguage);
    // }
